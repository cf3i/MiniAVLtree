...
- 单元/集成测试框架：当前未配置（仓库内未发现 `tests/`、`test/`、`__tests__/`、`spec/` 或其他原生测试文件；当前仅有 `scripts/run_issue_tests.sh` 的 issue 回归脚本框架，且 `issue_test/` 下未发现实际 `.sh` 用例）
- 静态检查工具：当前未配置（仓库内未发现 lint/type check/static analysis 配置、工具声明或版本锁定）
- 其他交付前命令：当前未配置（仓库内未发现项目原生构建/测试脚本、Makefile 目标或 CI 中定义的交付前命令）
